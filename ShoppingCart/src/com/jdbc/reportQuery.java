...
public class reportQuery {
	...
	public int reportQuery(String category , int year , int month){
		...
		try{
			...
			rs = stmt.executeQuery("SELECT sum(price) FROM sales WHERE EXTRACT(MONTH from sell) = '"+month+"' and EXTRACT(YEAR from sell) = '"+year+"' AND CATEGORY= '"+category+"'");
			rs.next();
			result = rs.getInt(1); 
		...
		System.out.println("from jdbc" + result);
		return result;
	}
}
